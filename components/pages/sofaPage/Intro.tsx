...
const locationStyle = "font-bold capitalize mr-[3px] ml-[3px]";
      ...
      <div className="flex justify-center max-mobile:flex-col-reverse gap-[50px]">
        <div className="mobile:w-[60%] flex flex-col gap-[10px]">
          <p>
            ...
            {location && (
              <span>
                in <span className={`${locationStyle}`}> {location}</span>
              </span>
            )}
            . These intricately designed pieces
            {location && (
              <span>
                in <span className={`${locationStyle}`}>, {location}</span>
              </span>
          ...
          <h2 className={`${h2Style}`}>
            Wooden Carved Sofa Set Suppliers{" "}
            {location && (
              <span>
                in <span className={`${locationStyle}`}>, {location}</span>
              </span>
          ...
          <p>
            ...
            showcasing the rich tradition of woodworking
            {location && (
              <span>
                in <span className={`${locationStyle}`}> {location}</span>
              </span>
            )}{" "}
            . We are one of the leading Wooden Carved Sofa Set Suppliers{" "}
            {location && (
              <span>
                in <span className={`${locationStyle}`}>, {location}</span>
              </span>
